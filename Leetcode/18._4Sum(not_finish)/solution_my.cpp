...
public:
	vector<vector<int>> fourSum(vector<int>& nums, int target) {
		...
		KSum_recursion(4, nums, target, res, stored, 0);
		printf("%lu\n", res.size());
		return res;
	}
private:
	void KSum_recursion(int K, vector<int> &nums, int target, vector<vector<int> >& res, vector<int>& stored, int start) {
		if (K == stored.size()) {
			int sum = 0;
			for (int i = 0; i < K; i++) sum += stored[i];
			if (target == sum) {
		...
		for (int i = start; i < nums.size(); i++) {
			int len_l = nums.size(), len_s = stored.size(), sum = 0;
			for (int j = 0; j < len_s; j++)  sum += stored[j];
			int min_val = sum, max_val = sum;
			for (int j = 0; j < (K-len_s); j++)  min_val += nums[i+j];
			for (int j = 0; j < (K-len_s); j++)  max_val += nums[len_l-1-j];
			if (target < min_val || target > max_val) return;
			stored.push_back(nums[i]);
			KSum_recursion(K, nums, target, res, stored, i + 1);
			stored.pop_back();
		}
	}
};
